...
// راه‌اندازی دکمه‌های مشاهده محصولات مرتبط
document.addEventListener('DOMContentLoaded', function() {
    // انتخاب تمام دکمه‌های مشاهده در بخش محصولات مرتبط
    const viewButtons = document.querySelectorAll('.horizontal-products .add-to-cart-btn');
    
    viewButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            
            // دریافت شناسه محصول از ویژگی data
            const productId = this.getAttribute('data-product-id');
            
            // دریافت اطلاعات محصول از نزدیک‌ترین کارت محصول
            ...
            // نمایش مودال جزئیات محصول
            showProductQuickView(productId, productName, productImage, productBrand, productPrice);
        });
    });
    
    // تابع نمایش مودال مشاهده سریع محصول
    function showProductQuickView(productId, productName, productImage, productBrand, productPrice) {
        ...
        if (!document.getElementById('quick-view-modal')) {
                    ...
                    <div class="modal-body">
                        <div class="product-quick-view">
                            ...
                            <div class="product-quick-details">
                                <h2 class="product-title" id="quick-view-title"></h2>
                                <div class="product-brand-info">
                                    <span class="brand-label">برند:</span>
                                    <span class="brand-value" id="quick-view-brand"></span>
                                </div>
                                <div class="product-price-info">
                                    <span class="price-label">قیمت:</span>
                                    <span class="price-value" id="quick-view-price"></span>
                                </div>
                                <div class="product-colors">
                                    <span class="colors-label">رنگ‌های موجود:</span>
                                    <div class="color-options" id="quick-view-colors">
                                        <!-- رنگ‌ها به صورت پویا اضافه می‌شوند -->
                                    </div>
                                </div>
                                <div class="product-sizes">
                                    <span class="sizes-label">سایزهای موجود:</span>
                                    <div class="size-options" id="quick-view-sizes">
                                        <!-- سایزها به صورت پویا اضافه می‌شوند -->
                                    </div>
                                </div>
                                <div class="product-actions">
                                    <button class="add-to-cart-btn primary-btn" id="quick-view-add-to-cart">
                                        <i class="ri-shopping-cart-line"></i>
                                        <span>افزودن به سبد خرید</span>
                                    </button>
                                    <a href="product-detail.html" class="view-details-btn secondary-btn" id="quick-view-details-link">
            ...
            document.getElementById('quick-view-backdrop').addEventListener('click', closeQuickView);
            
            // رویداد افزودن به سبد خرید
            document.getElementById('quick-view-add-to-cart').addEventListener('click', function() {
                // کد افزودن محصول به سبد خرید
        ...
        // به‌روزرسانی اطلاعات مودال
        document.getElementById('quick-view-image').src = productImage;
        document.getElementById('quick-view-image').alt = productName;
        document.getElementById('quick-view-title').textContent = productName;
        document.getElementById('quick-view-brand').textContent = productBrand;
        document.getElementById('quick-view-price').textContent = productPrice;
        
        // تنظیم شناسه محصول برای دکمه افزودن به سبد
        document.getElementById('quick-view-add-to-cart').setAttribute('data-product-id', productId);
        
        // تنظیم لینک مشاهده جزئیات کامل
        document.getElementById('quick-view-details-link').href = `product-detail.html?id=${productId}`;
        
        // اضافه کردن رنگ‌های نمونه
        const colorsContainer = document.getElementById('quick-view-colors');
        colorsContainer.innerHTML = ''; // پاک کردن رنگ‌های قبلی
        
        // دریافت رنگ‌های موجود از کارت محصول
        const productCard = document.querySelector(`.product-card .add-to-cart-btn[data-product-id="${productId}"]`).closest('.product-card');
        const colorDots = productCard.querySelectorAll('.product-colors .color-dot');
        
        colorDots.forEach(dot => {
            const color = document.createElement('div');
            color.className = 'color-option';
            color.style.backgroundColor = dot.style.backgroundColor;
            if (dot.style.border) {
                color.style.border = dot.style.border;
            }
            colorsContainer.appendChild(color);
        });
        
        // اضافه کردن سایزهای نمونه
        const sizesContainer = document.getElementById('quick-view-sizes');
        sizesContainer.innerHTML = ''; // پاک کردن سایزهای قبلی
        
        // سایزهای نمونه
        const sampleSizes = ['۴۰', '۴۱', '۴۲', '۴۳', '۴۴'];
    ...
    // تابع بستن مودال مشاهده سریع
    function closeQuickView() {
        const modal = document.getElementById('quick-view-modal');
        if (modal) {
            modal.classList.remove('active');
            document.body.style.overflow = ''; // فعال کردن مجدد اسکرول صفحه
        }
    }
    
    // تابع افزودن به سبد خرید
    function addToCart(productId) {
        // در اینجا کد افزودن محصول به سبد خرید را قرار دهید
        console.log(`محصول با شناسه ${productId} به سبد خرید اضافه شد`);
        
        // نمایش اعلان افزوده شدن به سبد
        showNotification('محصول با موفقیت به سبد خرید اضافه شد', 'success');
    }
    
    // تابع نمایش اعلان
    function showNotification(message, type = 'info') {
        // ایجاد اعلان اگر وجود ندارد
        ...
        // بستن اعلان با کلیک روی دکمه بستن
