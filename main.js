...
class PushNotification {
    ...
    async requestPermission() {
        ...
        if (this.permission === 'denied') {
            console.warn('کاربر قبلاً اجازه نوتیفیکیشن را رد کرده است');
            return false;
        }
    ...
    async send(title, options = {}) {
        ...
        try {
            ...
            notification.onclick = function() {
                const url = this.data?.url || window.location.origin;
    ...
    async sendNewProductNotification(productName, imageUrl = null) {
        return this.send('محصول جدید', {
            body: `محصول ${productName} به فروشگاه اضافه شد 😎`,
            icon: imageUrl || 'images/products/shoe1.webp',
            data: { url: 'محصولات.html?new=true' }
        });
    }
}

// این کد را در فایل main.js خود در بخش اسکریپت‌های document ready اضافه کنید
document.addEventListener('DOMContentLoaded', () => {
    ...
    // اضافه کردن رویداد کلیک به دکمه نوتیفیکیشن
    const notifButton = document.getElementById('notification-button');
    if (notifButton) {
        notifButton.addEventListener('click', async () => {
            await pushNotif.send('محصولات جدید', {
                body: 'محصولات جدید برای فروشگاه شارژ شد حتما یه سر بزن😎',
                // icon: 'ادیداس1.jpg'
            });
        });
    }
    
    // نمایش نوتیفیکیشن خوش‌آمدگویی با تأخیر چند ثانیه
    setTimeout(async () => {
        await pushNotif.send('به فروشگاه امیری خوش آمدید', {
            body: 'از تخفیف‌های ویژه و محصولات جدید ما دیدن کنید',
            // icon: 'ASS3.jpg',
            data: { url: 'محصولات.html?sale=true' }
        });
    }, 5000);
    
    // برنامه‌ریزی برای نمایش نوتیفیکیشن محصول جدید بعد از مدتی
    setTimeout(async () => {
        await pushNotif.sendNewProductNotification('نایک ایرفورس ۱ پرمیوم ۲۰۲۵', 'ASS3.jpg');
    }, 120000); // 2 دقیقه بعد
});
